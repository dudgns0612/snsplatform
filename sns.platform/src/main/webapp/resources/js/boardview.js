			...
			if(data.code == 1){
				...
				for(var i = 0 ; i < replyData.length; i++){
					...
					if(userNum == replyData[i].userNum){
						el += '<div class="replymodiftdelete"> <img class="replydelete" src="resources/img/delete.png" /> <img class="replymodify" src ="resources/img/edit.png" /> </div>'
					} 
					el += '</div><br/>';
				}
				replyList.html(el);
		...
		success : function(data){
			if(data.code == 1){
				...
				for(var i=0; i < len ; i++){
					...
					el += ' '+boardDateFormat+' ';
					el += ' </div>';	
					el += '<div><button type="button" class="board-detail-btn">상세보기</button></div>';
					el += '</div>';
...
// 게시글 상세보기
$(document).on('click','.imagenullboardcontent ,.boardcontent,.board-detail-btn',function(){
	if($(this).attr('class').match('board-detail-btn')){
		currentBoard = $(this).parents().siblings('.boardlist');
		boardNum = $(this).parents().siblings('input[type=hidden]').val();
	} else{
